...
3. **検索ページ** (`/src/app/search/page.tsx`):
   - 診療科選択（チェックボックス × 20）
   - 市町村選択（チェックボックス × 22）
   - キーワード入力（病院名）
   - 検索/リセットボタン
   ...
   - **URL パラメータによる検索状態の永続化**:
     - 検索実行時に URL を更新（`/search?categories=内科,外科&cities=飯田市&keyword=病院`）
     - ページ読み込み時に URL パラメータから初期値を復元
     - URL パラメータがある場合は自動的に検索を実行
     - 詳細ページからの戻るリンクで検索結果を保持
   - **Suspense 境界**: `useSearchParams()` 使用のため必須

4. **病院詳細ページ** (`/src/app/hospital/[id]/page.tsx`):
   - サーバーサイドレンダリング（async component）
   - Supabase から病院情報と診療時間を取得
   - HospitalCard コンポーネントで詳細表示
   - 「← 検索結果に戻る」リンク（検索パラメータ付き）
   - **検索状態の保持**: `searchParams` から検索条件を復元して戻るリンクを生成

5. **ヘッダー更新** (`/src/components/Common/Header.tsx`):
   - 「🔍 検索」ボタン追加
   - `/search` へのリンク
