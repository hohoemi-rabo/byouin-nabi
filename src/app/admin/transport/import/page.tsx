...
import { importTransportServices, exportTransportServicesCSV } from '@/app/admin/transport-actions';
...
export default function TransportImportPage() {
  ...
  const handleConfirmImport = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const res = await importTransportServices(formData);
        setResult(res);
      } catch (err) {
  ...
  const handleExport = () => {
    startTransition(async () => {
      try {
        const csv = await exportTransportServicesCSV();
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `transport_${new Date().toISOString().split('T')[0]}.csv`;
        a.click();
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">交通データ インポート/エクスポート</h1>

      <ConfirmModal
        isOpen={showConfirm}
        title="データの置換"
        message="既存の交通サービスデータを全て削除し、CSVの内容で置き換えます。\n\nこの操作は取り消せません。続行しますか？"
        confirmText="インポート実行"
        cancelText="キャンセル"
        type="danger"
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">現在登録されている交通サービスデータをCSVでダウンロードします。</p>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <p className="text-sm text-gray-600 mb-2">CSVファイルから交通サービスデータを一括登録します。</p>
        ...
        {selectedFile && (
          <Button variant="primary" onClick={handleImport} disabled={isPending} className="text-sm">
