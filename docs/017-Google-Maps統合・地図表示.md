...
- [ ] Google Cloud Console で Maps JavaScript API / Directions API / Geocoding API を有効化（API キー取得後）
...
- [ ] 「住所から座標を取得」ボタン — Geocoding API で住所→座標変換（API キー取得後）
- [ ] 既存病院データの一括座標変換機能（API キー取得後）
