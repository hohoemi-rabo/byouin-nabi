# 006 - 病院検索・表示機能

## 概要
診療科に基づいて病院を検索し、ユーザーに表示する機能を実装します。

## 目的
- 推奨診療科に対応する病院の検索
- 病院情報の見やすい表示
- 電話・地図へのアクセス

## タスク

### API エンドポイント作成
- [ ] GET /api/hospitals - 全病院取得
- [ ] GET /api/hospitals/search - 診療科での検索
- [ ] クエリパラメータ処理
- [ ] エラーハンドリング

### Supabase クエリ実装
- [ ] 診療科での絞り込みクエリ
- [ ] 並び順の実装（専門性優先）
- [ ] ページネーション（将来対応）

### 病院リストコンポーネント
- [ ] HospitalList コンポーネント作成
- [ ] HospitalCard コンポーネント作成
- [ ] Loading状態の表示
- [ ] エラー状態の表示
- [ ] データなし状態の表示

### 病院カード表示内容
- [ ] 病院名（大きく表示）
- [ ] 診療科（該当科を強調）
- [ ] 住所
- [ ] 電話番号（タップで発信）
- [ ] 診療時間
- [ ] 地図ボタン（Google Maps）
- [ ] 備考

## 実装例

```typescript
// src/app/api/hospitals/search/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { supabase } from '@/lib/supabase';

export async function GET(request: NextRequest) {
  try {
    const searchParams = request.nextUrl.searchParams;
    const categories = searchParams.get('categories')?.split(',') || [];

    if (categories.length === 0) {
      return NextResponse.json(
        { error: '診療科を指定してください' },
        { status: 400 }
      );
    }

    const { data: hospitals, error } = await supabase
      .from('hospitals')
      .select('*')
      .contains('category', categories)
      .order('name');

    if (error) {
      throw error;
    }

    return NextResponse.json({ hospitals });
  } catch (error) {
    console.error('Hospital search error:', error);
    return NextResponse.json(
      { error: '病院の検索に失敗しました' },
      { status: 500 }
    );
  }
}
```

```typescript
// src/components/HospitalList/HospitalCard.tsx
interface Hospital {
  id: string;
  name: string;
  category: string[];
  address: string;
  tel: string;
  city: string;
  opening_hours?: string;
  google_map_url?: string;
  note?: string;
}

export default function HospitalCard({
  hospital,
  highlightCategories = []
}: {
  hospital: Hospital;
  highlightCategories?: string[];
}) {
  return (
    <div className="border rounded-lg p-6 shadow-sm hover:shadow-md transition">
      <h3 className="text-2xl font-bold mb-2">{hospital.name}</h3>

      <div className="mb-3">
        <span className="text-sm text-gray-600">診療科：</span>
        <div className="flex flex-wrap gap-2 mt-1">
          {hospital.category.map(cat => (
            <span
              key={cat}
              className={`px-3 py-1 rounded-full text-sm ${
                highlightCategories.includes(cat)
                  ? 'bg-primary text-white font-bold'
                  : 'bg-gray-100'
              }`}
            >
              {cat}
            </span>
          ))}
        </div>
      </div>

      <div className="space-y-2 text-base">
        <p>📍 {hospital.address}</p>
        <p>
          📞 <a href={`tel:${hospital.tel}`} className="text-primary underline">
            {hospital.tel}
          </a>
        </p>
        {hospital.opening_hours && (
          <p>🕐 {hospital.opening_hours}</p>
        )}
      </div>

      <div className="mt-4 flex gap-3">
        {hospital.google_map_url && (
          <a
            href={hospital.google_map_url}
            target="_blank"
            rel="noopener noreferrer"
            className="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700"
          >
            地図で見る
          </a>
        )}
      </div>

      {hospital.note && (
        <p className="mt-3 text-sm text-gray-600 border-t pt-3">
          ℹ️ {hospital.note}
        </p>
      )}
    </div>
  );
}
```

## 受け入れ基準
- [ ] 診療科で病院が正しく絞り込まれる
- [ ] 病院情報が見やすく表示される
- [ ] 電話番号タップで発信できる
- [ ] 地図ボタンでGoogle Mapsが開く
- [ ] 該当する診療科が強調表示される
- [ ] データがない場合に適切なメッセージが表示される
- [ ] エラー時に適切なメッセージが表示される

## 依存関係
- 001-データベース設計・構築
- 005-診療科マッピング機能

## 関連ファイル
- `/home/masayuki/NextJs/byouin-nabi/src/app/api/hospitals/route.ts`
- `/home/masayuki/NextJs/byouin-nabi/src/app/api/hospitals/search/route.ts`
- `/home/masayuki/NextJs/byouin-nabi/src/components/HospitalList/`
- `/home/masayuki/NextJs/byouin-nabi/src/app/results/page.tsx`

## 備考
- 初期リリースでは全件表示（ページネーション未実装）
- 病院数が増えた場合はページネーションを追加
- 距離順ソートは将来対応（位置情報API使用）
