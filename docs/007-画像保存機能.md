# 007 - 画像保存機能

## 概要
症状説明文をスクリーンショットとして端末に保存する機能を実装します。

## 目的
- 症状説明文を画像として保存
- 家族や医師への共有を容易に
- サーバーには保存せず端末内のみ

## タスク

### html2canvas 導入
- [x] html2canvas パッケージのインストール
- [x] 型定義のインストール

### 画像保存コンポーネント
- [x] ImageSaveButton コンポーネント作成
- [x] スクリーンショット処理の実装
- [x] ダウンロード処理の実装
- [x] Loading状態の表示

### UI/UX
- [x] 保存完了メッセージの表示
- [x] エラーハンドリング
- [x] 保存後のオプションボタン
  - [x] 「もう一度保存する」
  - [x] 「トップページに戻る」

### スタイリング
- [x] 保存対象エリアのデザイン
- [x] 画像化時のフォーマット調整（白背景、影付き）
- [x] 背景色・余白の設定

## 実装例

```typescript
// src/components/SymptomResult/ImageSaveButton.tsx
'use client';

import { useState } from 'react';
import html2canvas from 'html2canvas';

export default function ImageSaveButton({ targetId }: { targetId: string }) {
  const [isLoading, setIsLoading] = useState(false);
  const [isSaved, setIsSaved] = useState(false);

  const handleSave = async () => {
    setIsLoading(true);

    try {
      const element = document.getElementById(targetId);
      if (!element) {
        throw new Error('保存対象が見つかりません');
      }

      const canvas = await html2canvas(element, {
        backgroundColor: '#ffffff',
        scale: 2, // 高解像度
      });

      // Canvas を Blob に変換
      canvas.toBlob((blob) => {
        if (!blob) {
          throw new Error('画像の生成に失敗しました');
        }

        // ダウンロード
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `症状説明_${new Date().getTime()}.png`;
        link.click();

        URL.revokeObjectURL(url);
        setIsSaved(true);
      });
    } catch (error) {
      console.error('Image save error:', error);
      alert('画像の保存に失敗しました。もう一度お試しください。');
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div>
      <button
        onClick={handleSave}
        disabled={isLoading}
        className="bg-success text-white px-6 py-3 rounded-lg text-lg font-bold min-h-[48px] hover:bg-success-dark disabled:opacity-50"
      >
        {isLoading ? '保存中...' : '📸 症状説明を画像で保存'}
      </button>

      {isSaved && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-green-800 font-bold mb-3">✓ 保存が完了しました</p>
          <div className="flex gap-3">
            <button
              onClick={() => window.location.href = '/'}
              className="bg-white border border-gray-300 px-4 py-2 rounded hover:bg-gray-50"
            >
              トップにもどる
            </button>
          </div>
        </div>
      )}
    </div>
  );
}
```

```typescript
// src/app/results/page.tsx での使用例
<div id="symptom-description" className="bg-white p-6 rounded-lg shadow">
  <SymptomDescription description={description} />
</div>

<ImageSaveButton targetId="symptom-description" />
```

## package.json 追加

```json
{
  "dependencies": {
    "html2canvas": "^1.4.1"
  },
  "devDependencies": {
    "@types/html2canvas": "^1.0.0"
  }
}
```

## 受け入れ基準
- [x] 症状説明文が画像として保存される
- [x] 画像の品質が十分である（PNG形式、白背景）
- [x] 保存完了メッセージが表示される
- [x] エラー時に適切なメッセージが表示される
- [x] 保存後のオプションが表示される
- [x] サーバーにデータが送信されない（クライアント側のみ）

## 依存関係
- 004-症状説明文生成機能

## 関連ファイル
- `/home/masayuki/NextJs/byouin-nabi/src/components/SymptomResult/ImageSaveButton.tsx`
- `/home/masayuki/NextJs/byouin-nabi/src/app/results/page.tsx`
- `/home/masayuki/NextJs/byouin-nabi/package.json`

## 備考
- html2canvas はクライアントサイドでのみ動作（'use client' 必須）
- iOS Safari では挙動が異なる場合があるため、十分なテストが必要
- 将来的にLINE共有機能を追加予定（Web Share API 使用）
- 画像フォーマットはPNG（透過なし、白背景）
